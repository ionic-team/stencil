...
import { BOOLEAN_CLI_FLAGS, BOOLEAN_STRING_CLI_FLAGS, NUMBER_CLI_FLAGS, STRING_ARRAY_CLI_FLAGS, STRING_CLI_FLAGS, } from '../config-flags';
...
describe('parseFlags', () => {
    ...
    describe.each(BOOLEAN_STRING_CLI_FLAGS)('boolean-string flag - %s', (cliArg) => {
        ...
        it(`parses a boolean-string flag as a falsy boolean with "no" arg - --no${cliArg.charAt(0).toUpperCase() + cliArg.slice(1)}`, () => {
            const negativeFlag = '--no' + cliArg.charAt(0).toUpperCase() + cliArg.slice(1);
            const flags = parseFlags([negativeFlag]);
            expect(flags.headless).toBe(false);
            expect(flags.knownArgs).toEqual([negativeFlag]);
    ...
    describe.each(['info', 'warn', 'error', 'debug'])('logLevel %s', (level) => {
    ...
    describe.each(STRING_ARRAY_CLI_FLAGS)('should parse string flag %s', (cliArg) => {
...
//# sourceMappingURL=parse-flags.spec.js.map