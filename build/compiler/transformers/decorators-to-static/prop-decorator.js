...
import { convertValueToLiteral, createStaticGetter, getAttributeTypeInfo, isMemberPrivate, resolveType, retrieveTsDecorators, retrieveTsModifiers, serializeSymbol, tsPropDeclNameAsString, typeToString, validateReferences, } from '../transform-utils';
 ...
 */
export const propDecoratorsToStatic = (diagnostics, decoratedProps, typeChecker, program, newMembers) => {
 ...
 */
const parsePropDecorator = (diagnostics, typeChecker, program, prop) => {
    var _a;
    ...
    const staticProp = ts.factory.createPropertyAssignment(ts.factory.createStringLiteral(propName), convertValueToLiteral(propMeta));
    return staticProp;
};
/**
...
const getComplexType = (typeChecker, node, type, program) => {
    ...
    return {
        ...
        nodeType ? node : typeChecker.typeToTypeNode(type, undefined, undefined), node.getSourceFile(), typeChecker, program),
    };
};
/**
