 ...
 */
export const createTerminalLogger = (loggerSys) => {
    ...
    const formatMemoryUsage = (message) => {
        ...
        if (mem > 0) {
            message.push(dim(` MEM: ${(mem / 1000000).toFixed(1)}MB`));
        }
    };
     ...
     */
    const highlightError = (errorLine, errorCharStart, errorLength) => {
 ...
 */
export const shouldLog = (currentSetting, messageLevel) => LOG_LEVELS.indexOf(messageLevel) >= LOG_LEVELS.indexOf(currentSetting);
/**
 ...
 */
export const wordWrap = (msg, columns) => {
    const lines = [];
    const words = [];
    msg.forEach((m) => {
        if (m === null) {
