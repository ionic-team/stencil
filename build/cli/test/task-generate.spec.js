...
import { getBoilerplateByExtension, taskGenerate } from '../task-generate';
 ...
 */
async function silentGenerate(config) {
...
describe('generate task', () => {
    ...
    it('should exit with an error if no `configPath` is supplied', async () => {
        ...
        expect(errorSpy).toHaveBeenCalledWith('Please run this command in your root directory (i. e. the one containing stencil.config.ts).');
    });
    ...
    it.each([true, false])('should create a directory for the generated components', async (includeTests) => {
        ...
        expect(createDirSpy).toHaveBeenCalledWith(includeTests ? `${config.srcDir}/components/my-component/test` : `${config.srcDir}/components/my-component`, { recursive: true });
    });
    it('should generate the files the user picked', async () => {
        ...
        userChoices.forEach((file) => {
            expect(writeFileSpy).toHaveBeenCalledWith(file.path, getBoilerplateByExtension('my-component', file.extension, true));
        });
    });
    it('should error without writing anything if a to-be-generated file is already present', async () => {
        ...
        await silentGenerate(config);
        expect(errorSpy).toHaveBeenCalledWith('Generating code would overwrite the following files:', '\t/src/components/my-component/my-component.tsx', '\t/src/components/my-component/my-component.css', '\t/src/components/my-component/test/my-component.spec.tsx', '\t/src/components/my-component/test/my-component.e2e.ts');
        expect(config.sys.exit).toHaveBeenCalledWith(1);
    });
});
//# sourceMappingURL=task-generate.spec.js.map