...
describe('validateDevServer', () => {
    ...
    it.each(['https://localhost', 'http://localhost', 'https://localhost/', 'http://localhost/', 'localhost/'])('should remove extraneous stuff from address %p', (address) => {
    ...
    it.each(['https://subdomain.stenciljs.com:3000', 'localhost:3000/', 'localhost:3000'])('should override port in address with port property', (address) => {
    ...
    it.each([1, []])('should default historyApiFallback when an invalid value (%s) is provided', (badValue) => {
        ...
        inputConfig.devServer = { ...inputDevServerConfig, historyApiFallback: badValue };
        const { config } = validateConfig(inputConfig, mockLoadConfigInit());
        expect(config.devServer.historyApiFallback).toBeDefined();
        expect(config.devServer.historyApiFallback.index).toBe('index.html');
    });
...
//# sourceMappingURL=validate-dev-server.spec.js.map