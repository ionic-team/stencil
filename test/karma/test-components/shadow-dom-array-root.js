...
const ShadowDomArrayRoot$1 = class extends HTMLElement {
  ...
  render() {
    return h(
      'div',
      null,
      h('button', { onClick: this.addValue.bind(this) }, 'Add Value'),
      h('shadow-dom-array', { values: this.values, class: 'results1' })
    );
  }
};

const ShadowDomArrayRoot = /*@__PURE__*/ proxyCustomElement(ShadowDomArrayRoot$1, [
  0,
  ...
  components.forEach((cmp) => {
    switch (cmp) {
      ...
      case 'shadow-dom-array':
        ...
        if (!customElements.get(tagName)) {
          const ShadowDomArray$1 = /*@__PURE__*/ proxyCustomElement(ShadowDomArray, [
            0,
            'shadow-dom-array-root',
            { values: [32] },
          ]);
