...
describe('url-paths', () => {
    describe('isRemoteUrl', () => {
        it.each(['http://domain.com/file.txt', 'HTTP://domain.com/file.txt'])("returns true for the http protocol '%s'", (url) => {
            expect(isRemoteUrl(url)).toBe(true);
        });
        it.each(['https://domain.com/file.txt', 'HTTPS://domain.com/file.txt'])("returns true for the https protocol '%s'", (url) => {
            expect(isRemoteUrl(url)).toBe(true);
        });
...
//# sourceMappingURL=url-paths.spec.js.map