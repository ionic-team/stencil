...
@Component({
  tag: 'slot-fallback-root-polyfill'
})
export class SlotFallbackRoot {
  ...
  render() {
    return [
      <button onClick={this.changeFallbackContent.bind(this)} class='change-fallback-content'>Change Fallback Slot Content</button>,

      <button onClick={this.changeLightDom.bind(this)} class='change-light-dom'>
        {this.inc % 2 === 0 ?
          'Use light dom content' :
          'Use fallback slot content'
        }
      </button>,

      <button onClick={this.changeSlotContent.bind(this)} class='change-slot-content'>Change Slot Content</button>,

      <slot-fallback-polyfill inc={this.fallbackInc} class='results1'>
        {this.inc % 2 !== 0 ?
          [
