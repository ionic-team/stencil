...
export const componentDecoratorToStatic = (config, typeChecker, diagnostics, cmpNode, newMembers, componentDecorator) => {
    const [componentOptions] = getDecoratorParameters(componentDecorator, typeChecker);
    if (!componentOptions) {
        return;
    }
...
const validateComponent = (config, diagnostics, typeChecker, componentOptions, cmpNode, componentDecorator) => {
    ...
    if (!config._isTesting) {
        const nonTypeExports = typeChecker
            ...
            .filter((symbol) => (symbol.flags & (ts.SymbolFlags.Interface | ts.SymbolFlags.TypeAlias | ts.SymbolFlags.Enum)) === 0)
