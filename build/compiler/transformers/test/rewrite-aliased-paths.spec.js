 ...
 */
async function pathTransformTranspile(component, inputFileName = 'module.tsx') {
    const compilerContext = mockCompilerCtx();
    ...
    await compilerContext.fs.writeFile(path.join(config.rootDir, 'name/space/keyboard.ts'), 'export const keyboard = "keyboard"');
    return transpileModule(component, null, compilerContext, [rewriteAliasedSourceFileImportPaths], [], [rewriteAliasedDTSImportPaths], mockPathsConfig, normalizePath(path.join(config.rootDir, inputFileName)));
}
describe('rewrite alias module paths transform', () => {
    ...
    `);
        expect(await formatCode(t.outputText)).toBe(await formatCode('import { foo } from "./name/space";export class CmpA { render() { return h("some-cmp", null, foo("bar")); }}'));
    });
    ...
    `);
        expect(await formatCode(t.outputText)).toBe(await formatCode('import { foo } from "./name/space/subdir";export class CmpA { render() { return h("some-cmp", null, foo("bar")); }}'));
    });
    ...
    `);
        expect(await formatCode(t.declarationOutputText)).toBe(await formatCode('import { Foo } from "./name/space";export declare class CmpA { field: Foo; render(): any;}'));
    });
    ...
    `);
        expect(await formatCode(t.declarationOutputText)).toBe(await formatCode('import { Foo } from "./name/space/subdir";export declare function fooUtil(foo: Foo): Foo;'));
    });
    ...
    `);
        expect(await formatCode(t.declarationOutputText)).toBe(await formatCode('import { Foo } from "./name/space/subdir";import { Bar } from "./name/space";export declare function fooUtil(foo: Foo): Bar;'));
    });
    ...
    `, 'name/component.tsx');
        ...
        expect(await formatCode(t.outputText)).toBe(await formatCode('import { foo } from "./space";export class CmpA { render() { return h("some-cmp", null, foo("bar")); }}'));
    });
    ...
    `, 'name/component.tsx');
        ...
        expect(await formatCode(t.outputText)).toBe(await formatCode(`import { foo } from "./space/subdir";
        export class CmpA {
          render() {
    ...
    `);
        expect(await formatCode(t.outputText)).toBe(await formatCode('import { foo } from "./name/space/keyboard";export class CmpA { render() { return h("some-cmp", null, foo("bar")); }}'));
    });
});
//# sourceMappingURL=rewrite-aliased-paths.spec.js.map