...
export const defaultConfig = (sys) => sys.resolvePath(`${sys.homeDir()}/.ionic/${isTest() ? 'tmp-config.json' : 'config.json'}`);
export const defaultConfigDirectory = (sys) => sys.resolvePath(`${sys.homeDir()}/.ionic`);
/**
 ...
 */
export async function readConfig(sys) {
 ...
 */
export async function writeConfig(sys, config) {
    let result = false;
    try {
 ...
 * @returns boolean If the command was successful
 */
export async function updateConfig(sys, newOptions) {
