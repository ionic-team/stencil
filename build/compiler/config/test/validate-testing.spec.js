...
describe('validateTesting', () => {
    ...
    describe('browserHeadless', () => {
        ...
        describe('`testing` configuration', () => {
            ...
            it.each([false, true, 'new'])('uses %s browserHeadless mode from testing config', (browserHeadlessValue) => {
    ...
    describe('screenshotConnector', () => {
        ...
        it('sets screenshotConnector if a non-string is provided', () => {
            ...
            userConfig.testing = { screenshotConnector: true };
            const { config } = validateConfig(userConfig, mockLoadConfigInit());
    ...
    describe('setupFilesAfterEnv', () => {
        it.each([null, true])('forces a non-array (%s) of setup files to a default', (nonSetupFiles) => {
            ...
            userConfig.testing = {
                // the nature of this test requires a non-string value, hence the type assertion
                setupFilesAfterEnv: nonSetupFiles,
            };
            const { config } = validateConfig(userConfig, mockLoadConfigInit());
        ...
        it.each([[[]], [['mock-setup-file.js']]])("prepends stencil's default file to an array: %s", (setupFilesAfterEnv) => {
            userConfig.flags = { ...flags, e2e: true };
            userConfig.testing = {
    ...
    describe('allowableMismatchedRatio', () => {
        it.each([-0, 0, 0.5, 1.0])('does nothing if a value between 0 and 1 is provided (%s)', (allowableMismatchedRatio) => {
            userConfig.flags = { ...flags, e2e: true };
            userConfig.testing = {
        ...
        it.each([-1, -0.1, 1.1, 2])('creates an error if a number outside 0 and 1 is provided (%s)', (allowableMismatchedRatio) => {
            userConfig.flags = { ...flags, e2e: true };
            userConfig.testing = {
                allowableMismatchedRatio,
            };
        ...
        it.each([true, null])('does nothing when a non-number (%s) is provided', (allowableMismatchedRatio) => {
            ...
            userConfig.testing = {
                // the nature of this test requires using a non-number, hence th type assertion
                allowableMismatchedRatio: allowableMismatchedRatio,
            };
            const { config } = validateConfig(userConfig, mockLoadConfigInit());
            expect(config.testing.allowableMismatchedRatio).toBe(allowableMismatchedRatio);
        });
    ...
    describe('pixelmatchThreshold', () => {
        ...
        it.each([-0.1, -1, 1.1, 2])('creates an error if a number outside 0 and 1 is provided (%s)', (pixelmatchThreshold) => {
            userConfig.flags = { ...flags, e2e: true };
            userConfig.testing = {
                pixelmatchThreshold,
            };
    ...
    describe('testRegex', () => {
        ...
        beforeEach(() => {
            ...
            const testRegexSetting = testConfig === null || testConfig === void 0 ? void 0 : testConfig.testRegex;
            if (!testRegexSetting) {
    ...
    describe('waitBeforeScreenshot', () => {
        ...
        it.each([true, null])('defaults to a reasonable value if a non-number (%s) is provided', (waitBeforeScreenshot) => {
            ...
            userConfig.testing = {
                ...
                pixelmatchThreshold: waitBeforeScreenshot,
            };
            const { config } = validateConfig(userConfig, mockLoadConfigInit());
...
//# sourceMappingURL=validate-testing.spec.js.map