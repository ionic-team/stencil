...
describe('proxy-custom-element-function', () => {
    ...
    let compilerCtx;
    let transformOpts;
    let getModuleFromSourceFileSpy;
    let createClassMetadataProxySpy;
    beforeEach(() => {
        ...
        getModuleFromSourceFileSpy.mockImplementation((_compilerCtx, _tsSourceFile) => {
        ...
        createClassMetadataProxySpy.mockImplementation((_compilerMeta, clazz) => ts.factory.createCallExpression(ts.factory.createIdentifier(PROXY_CUSTOM_ELEMENT), [], [clazz, ts.factory.createTrue()]));
    });
    afterEach(() => {
    ...
    describe('proxyCustomElement()', () => {
        it('imports PROXY_CUSTOM_ELEMENT', () => {
            ...
            expect(transpiledModule.outputText).toContain(`import { proxyCustomElement as __stencil_proxyCustomElement } from "@stencil/core";`);
        });
        it('wraps a class initializer in a proxyCustomElement call', async () => {
            const code = `const ${componentClassName} = class extends HTMLElement {};`;
            const transformer = proxyCustomElement(compilerCtx, transformOpts);
            const transpiledModule = transpileModule(code, null, compilerCtx, [], [transformer]);
            expect(await formatCode(transpiledModule.outputText)).toContain(await formatCode(`const ${componentClassName} = /*@__PURE__*/ __stencil_proxyCustomElement(class ${componentClassName} extends HTMLElement {}, true);`));
        });
        it('wraps an exported class initializer in a proxyCustomElement call', async () => {
            ...
            const transformer = proxyCustomElement(compilerCtx, transformOpts);
            const transpiledModule = transpileModule(code, null, compilerCtx, [], [transformer]);
            expect(await formatCode(transpiledModule.outputText)).toContain(await formatCode(`export const ${componentClassName} = /*@__PURE__*/ __stencil_proxyCustomElement(class ${componentClassName} extends HTMLElement {}, true);`));
        });
        describe('multiple variable declarations', () => {
            it('wraps a class initializer properly when a variable declaration precedes it', async () => {
                ...
                const transformer = proxyCustomElement(compilerCtx, transformOpts);
                const transpiledModule = transpileModule(code, null, compilerCtx, [], [transformer]);
                expect(await formatCode(transpiledModule.outputText)).toContain(await formatCode(`const foo = 'hello world!', ${componentClassName} = /*@__PURE__*/ __stencil_proxyCustomElement(class ${componentClassName} extends HTMLElement {}, true);`));
            });
            it('wraps a class initializer properly when it precedes another variable declaration', async () => {
                ...
                const transformer = proxyCustomElement(compilerCtx, transformOpts);
                const transpiledModule = transpileModule(code, null, compilerCtx, [], [transformer]);
                expect(await formatCode(transpiledModule.outputText)).toContain(await formatCode(`const ${componentClassName} = /*@__PURE__*/ __stencil_proxyCustomElement(class ${componentClassName} extends HTMLElement {}, true), foo = 'hello world!';`));
            });
            it('wraps a class initializer properly in the middle of multiple variable declarations', async () => {
                ...
                const transformer = proxyCustomElement(compilerCtx, transformOpts);
                const transpiledModule = transpileModule(code, null, compilerCtx, [], [transformer]);
                expect(await formatCode(transpiledModule.outputText)).toContain(await formatCode(`const foo = 'hello world!', ${componentClassName} = /*@__PURE__*/ __stencil_proxyCustomElement(class ${componentClassName} extends HTMLElement {}, true), bar = 'goodbye?';`));
            });
        });
    });
...
//# sourceMappingURL=proxy-custom-element-function.spec.js.map