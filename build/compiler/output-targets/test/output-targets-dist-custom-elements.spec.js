...
describe('Custom Elements output target', () => {
    ...
    it.each([[[]], [[{ type: 'dist' }]]])('should return early if no appropriate output target (%j)', async (outputTargets) => {
        const { config, compilerCtx, buildCtx, bundleCustomElementsSpy } = setup();
...
`);
    ...
    describe('bundleCustomElements', () => {
        it('should set a diagnostic if no `dir` prop on the output target', async () => {
            ...
            const outputTarget = { type: DIST_CUSTOM_ELEMENTS };
    ...
    describe('addCustomElementInputs', () => {
        ...
        describe('no defined CustomElementsExportBehavior', () => {
            it("doesn't re-export components from the index.js barrel file", () => {
                ...
                addCustomElementInputs(buildCtx, bundleOptions, config.outputTargets[0]);
                expect(bundleOptions.loader['\0core']).toEqual(`import { globalScripts } from '${STENCIL_APP_GLOBALS_ID}';
export { getAssetPath, setAssetPath, setNonce, setPlatformOptions } from '${STENCIL_INTERNAL_CLIENT_ID}';
export * from '${USER_INDEX_ENTRY_ID}';
...
`);
        ...
        describe('CustomElementsExportBehavior.SINGLE_EXPORT_MODULE', () => {
            beforeEach(() => {
                config.outputTargets[0].customElementsExportBehavior =
...
`);
        ...
        describe('CustomElementsExportBehavior.BUNDLE', () => {
            beforeEach(() => {
                config.outputTargets[0].customElementsExportBehavior = 'bundle';
            });
...
//# sourceMappingURL=output-targets-dist-custom-elements.spec.js.map