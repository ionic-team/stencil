...
jest.mock('@utils', () => {
    ...
    return {
        __esModule: true,
        ...originalUtils,
        resolve: (...pathSegments) => pathSegments.pop(),
    };
});
...
describe('stencil-types', () => {
    describe('updateTypeMemberNames', () => {
        ...
        beforeEach(() => {
            ...
            dirnameSpy.mockImplementation((path) => path);
        });
        afterEach(() => {
        ...
        describe('no type transformations', () => {
            it('returns the provided type when no type references exist', () => {
                ...
                const actualTypeName = updateTypeIdentifierNames({}, {}, stubComponentCompilerMeta().sourceFilePath, expectedTypeName);
            ...
            it('returns the provided type when no type reference matches are found in the import data', () => {
                ...
                const expectedTypeName = 'CustomType';
                const actualTypeName = updateTypeIdentifierNames(typeReferences, {}, stubComponentCompilerMeta().sourceFilePath, expectedTypeName);
                expect(actualTypeName).toBe(expectedTypeName);
            });
            ...
            it("does not change a simple type when there's no import name to alias", () => {
                ...
                const actualTypeName = updateTypeIdentifierNames(typeReferences, typeImports, componentCompilerMeta.sourceFilePath, initialType);
        ...
        describe('path resolution', () => {
             ...
             */
            const expectTypeTransformForPath = (basePath, initialType, expectedType) => {
                const typePath = `${basePath}/my-types`;
                const componentCompilerMeta = stubComponentCompilerMeta({
                    sourceFilePath: `${basePath}/my-component.tsx`,
                });
         ...
         */
        const expectTypeIsTransformed = (initialType, expectedType, typeMemberNames) => {
            const basePath = '~/some/stubbed/path';
            const componentCompilerMeta = stubComponentCompilerMeta({
                sourceFilePath: `${basePath}/my-component.tsx`,
            });
...
//# sourceMappingURL=stencil-types.spec.js.map