 ...
 */
export const addDefineCustomElementFunctions = (compilerCtx, components, outputTarget) => {
    return () => {
        return (tsSourceFile) => {
            ...
            if (moduleFile.cmps.length) {
                ...
                const customElementsDefineCallExpression = ts.factory.createCallExpression(ts.factory.createPropertyAccessExpression(ts.factory.createIdentifier('customElements'), 'define'), undefined, [ts.factory.createIdentifier('tagName'), ts.factory.createIdentifier(principalComponent.componentClassName)]);
...
const createCustomElementsDefineCase = (tagName, actionExpression) => {
    return ts.factory.createCaseClause(ts.factory.createStringLiteral(tagName), [
        ts.factory.createIfStatement(ts.factory.createPrefixUnaryExpression(ts.SyntaxKind.ExclamationToken, ts.factory.createCallExpression(ts.factory.createPropertyAccessExpression(ts.factory.createIdentifier('customElements'), 'get'), undefined, [ts.factory.createIdentifier('tagName')])), ts.factory.createBlock([ts.factory.createExpressionStatement(actionExpression)])),
...
const addDefineCustomElementFunction = (tagNames, newStatements, caseStatements) => {
    const newExpression = ts.factory.createFunctionDeclaration([ts.factory.createModifier(ts.SyntaxKind.ExportKeyword)], undefined, ts.factory.createIdentifier('defineCustomElement'), undefined, [], undefined, ts.factory.createBlock([
        ts.factory.createIfStatement(ts.factory.createStrictEquality(ts.factory.createTypeOfExpression(ts.factory.createIdentifier('customElements')), ts.factory.createStringLiteral('undefined')), ts.factory.createBlock([ts.factory.createReturnStatement()])),
        ts.factory.createVariableStatement(undefined, ts.factory.createVariableDeclarationList([
            ts.factory.createVariableDeclaration('components', undefined, undefined, ts.factory.createArrayLiteralExpression(tagNames.map((tagName) => ts.factory.createStringLiteral(tagName)))),
        ], ts.NodeFlags.Const)),
        ts.factory.createExpressionStatement(ts.factory.createCallExpression(ts.factory.createPropertyAccessExpression(ts.factory.createIdentifier('components'), 'forEach'), undefined, [
            ts.factory.createArrowFunction(undefined, undefined, [
                ts.factory.createParameterDeclaration(undefined, undefined, ts.factory.createIdentifier('tagName'), undefined, undefined),
            ], undefined, ts.factory.createToken(ts.SyntaxKind.EqualsGreaterThanToken), ts.factory.createBlock([
                ts.factory.createSwitchStatement(ts.factory.createIdentifier('tagName'), ts.factory.createCaseBlock(caseStatements)),
            ])),
        ])),
    ], true));
...
function createAutoDefinitionExpression(componentName) {
    return ts.factory.createExpressionStatement(ts.factory.createCallExpression(ts.factory.createIdentifier('defineCustomElement'), undefined, [
        ts.factory.createIdentifier(componentName),
    ]));
}
//# sourceMappingURL=add-define-custom-element-function.js.map