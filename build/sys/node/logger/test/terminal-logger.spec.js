...
import { LOG_LEVELS } from '../../../../declarations';
...
describe('terminal-logger', () => {
    describe('shouldLog helper', () => {
        it.each(LOG_LEVELS)("should log errors at level '%s'", (currentLevel) => {
    ...
    describe('basic logging functionality', () => {
        ...
        function setup() {
            ...
            loggerSys.memoryUsage = jest.fn().mockReturnValue(10000000);
        ...
        describe('timespans', () => {
            ...
            describe('debug timespan', function () {
                it('supports passing a debug flag', function () {
                    ...
                    jest.advanceTimersByTime(10000);
                    timespan.finish('finish the timespan');
                    expect(logMock).toHaveBeenNthCalledWith(1, `${cyan('[32:32.0]')}  start the timespan ${dim('...')} ${dim(' MEM: 10.0MB')}`);
                    expect(logMock).toHaveBeenNthCalledWith(2, `${cyan('[32:42.0]')}  finish the timespan ${dim('in 10.00 s')} ${dim(' MEM: 10.0MB')}`);
                });
                ...
                it.each(['info', 'error', 'warn'])("shouldn't write to the console when logLevel is '%s' (less verbose than 'debug')", (level) => {
                    const { logger, logMock } = setup();
            ...
            it("doesn't report an exact time if it's less than 1ms", function () {
                ...
                timespan.finish('finish the timespan');
                expect(logMock).toHaveBeenNthCalledWith(1, `${dim('[32:32.0]')}  start the timespan ${dim('...')}`);
                expect(logMock).toHaveBeenNthCalledWith(2, `${dim('[32:32.0]')}  finish the timespan ${dim('in less than 1 ms')}`);
            });
...
//# sourceMappingURL=terminal-logger.spec.js.map