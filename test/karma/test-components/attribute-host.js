...
const AttributeHost$1 = class extends HTMLElement {
  constructor() {
    super();
    this.__registerHost();
...
const AttributeHost = /*@__PURE__*/ proxyCustomElement(AttributeHost$1, [0, 'attribute-host', { attrsAdded: [32] }]);
...
const defineCustomElement = (tagRename) => {
  ...
  components.forEach((cmp) => {
    switch (cmp) {
      case 'attribute-host':
        tagName = 'attribute-host';
        if (tagRename) {
          tagName = tagRename(tagName);
        }
        if (!customElements.get(tagName)) {
          customElements.define(tagName, AttributeHost);
        }
        break;
    }
  });
