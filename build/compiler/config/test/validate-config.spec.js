...
describe('validation', () => {
    ...
    it.each([DOCS_JSON, DOCS_CUSTOM, DOCS_README, DOCS_VSCODE])('should not add "%s" output target by default', (targetType) => {
        const { config } = validateConfig(userConfig, bootstrapConfig);
    ...
    it('should change a single watchIgnoredRegex to an array', () => {
        ...
        expect(config.watchIgnoredRegex[0]).toEqual(/\.(gif|jpe?g|png)$/i);
        expect(isWatchIgnorePath(config, '/some/image.gif')).toBe(true);
        expect(isWatchIgnorePath(config, '/some/typescript.ts')).toBe(false);
    });
    it('should clean up valid watchIgnoredRegex', () => {
        userConfig.watchIgnoredRegex = [/\.(gif|jpe?g)$/i, null, 'me-regex', /\.(png)$/i];
        const { config } = validateConfig(userConfig, bootstrapConfig);
        expect(config.watchIgnoredRegex).toHaveLength(2);
        expect(config.watchIgnoredRegex[0]).toEqual(/\.(gif|jpe?g)$/i);
        expect(config.watchIgnoredRegex[1]).toEqual(/\.(png)$/i);
        expect(isWatchIgnorePath(config, '/some/image.gif')).toBe(true);
    ...
    describe('validatePrimaryPackageOutputTarget', () => {
        ...
        it.each([true, false])('should set validatePrimaryPackageOutputTarget to %p', (validatePrimaryPackageOutputTarget) => {
