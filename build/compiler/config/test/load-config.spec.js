...
describe('load config', () => {
    ...
    it("merges a user's configuration with a stencil.config file on disk", async () => {
        ...
        expect(actualConfig.flags).toEqual({ dev: true });
    ...
    it('uses the provided config path when no initial config provided', async () => {
        ...
        expect(actualConfig.flags).toEqual({});
    });
    describe('empty initialization argument', () => {
        ...
        it('errors that a tsconfig file could not be created when "initTsConfig" isn\'t present', async () => {
            ...
            expect(loadedConfig.diagnostics[0]).toEqual({
                absFilePath: undefined,
                header: 'Missing tsconfig.json',
                level: 'error',
                lines: [],
                messageText: `Unable to load TypeScript config file. Please create a "tsconfig.json" file within the "${normalizePath(path.dirname(configPath))}" directory.`,
    ...
    describe('no initialization argument', () => {
        it('errors that a tsconfig file cannot be found', async () => {
            ...
            expect(loadConfigResults.diagnostics[0]).toEqual({
                absFilePath: undefined,
                header: 'Missing tsconfig.json',
                level: 'error',
                lines: [],
                messageText: expect.stringMatching(`Unable to load TypeScript config file. Please create a "tsconfig.json" file within the`),
                relFilePath: undefined,
                type: 'build',
            });
        });
...
//# sourceMappingURL=load-config.spec.js.map