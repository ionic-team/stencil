...
import { filesChanged, hasHtmlChanges, hasScriptChanges, hasStyleChanges, isWatchIgnorePath, scriptsAdded, scriptsDeleted, } from '../fs-watch/fs-watch-rebuild';
...
export const createWatchBuild = async (config, compilerCtx) => {
     ...
     */
    const onFsChange = (filePath, eventKind) => {
        if (tsWatchProgram && !isWatchIgnorePath(config, filePath)) {
            ...
            config.logger.debug(`WATCH_BUILD::fs_event_change - type=${eventKind}, path=${filePath}, time=${new Date().getTime()}`);
     ...
     * @param eventKind The type of file change that occurred (update, add, delete)
     */
    const onDirChange = (filePath, eventKind) => {
        if (eventKind != null) {
...
//# sourceMappingURL=watch-build.js.map