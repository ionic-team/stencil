...
export class Logger {
  ...
  constructor(private opt: CompilerOptions, scope: string) {
    this.start = Date.now();
    this.scope = scope;
   ...
   */
  static debug(opt: CompilerOptions, ...msg: any[]) {
    if (opt.debugMode) {
...
export function runTypeScriptDiagnostics(opts: CompilerOptions, tsDiagnostics: ts.Diagnostic[]) {
...
function loadDiagnostic(opts: CompilerOptions, tsDiagnostic: ts.Diagnostic) {
  const d: Diagnostic = {
    level: 'error',
    type: 'typescript',
    language: 'typescript',
