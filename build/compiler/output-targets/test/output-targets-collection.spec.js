...
describe('Dist Collection output target', () => {
    let mockConfig;
    let mockedBuildCtx;
    let mockedCompilerCtx;
    let changedModules;
    let mapImportPathSpy;
    const mockTraverse = jest.fn().mockImplementation((source) => source);
    ...
    describe('transform aliased import paths', () => {
        ...
        it.each([true, false])('calls function to transform aliased import paths when the output target config flag is `%s`', async (transformAliasedImportPaths) => {
...
//# sourceMappingURL=output-targets-collection.spec.js.map