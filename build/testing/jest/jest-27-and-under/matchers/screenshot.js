export function toMatchScreenshot(compare, opts = {}) {
    ...
    if (typeof compare.mismatchedPixels !== 'number') {
        throw new Error(`expect toMatchScreenshot() value is not a valid screenshot compare object - 'mismatchedPixels' has type '${typeof compare.mismatchedPixels}', but should be a number`);
    }
    if (typeof compare.deviceScaleFactor !== 'number') {
        throw new Error(`expect toMatchScreenshot() value is not a valid screenshot compare object - 'deviceScaleFactor' has type '${typeof compare.deviceScaleFactor}', but should be a number`);
    }
    ...
    if (typeof opts.allowableMismatchedRatio === 'number') {
        ...
        const mismatchedRatio = compare.mismatchedPixels /
