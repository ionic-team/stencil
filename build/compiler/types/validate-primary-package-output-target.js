 ...
 */
export const validatePrimaryPackageOutputTarget = (config, compilerCtx, buildCtx) => {
    if (config.validatePrimaryPackageOutputTarget) {
        const eligiblePrimaryTargets = [];
        const nonPrimaryTargets = [];
        ...
        if (eligiblePrimaryTargets.length) {
            ...
            if (targetsMarkedToValidate.length) {
                ...
                if (targetsMarkedToValidate.length > 1) {
                    logValidationWarning(buildCtx, `Your Stencil config has multiple output targets with 'isPrimaryPackageOutputTarget: true'. Stencil does not support validating 'package.json' fields for multiple output targets. Please remove the 'isPrimaryPackageOutputTarget' flag from all but one of the following output targets: ${targetsMarkedToValidate
                        .map((ref) => ref.type)
                        .join(', ')}. For now, Stencil will use the first primary target it finds. You can read more about primary package output targets in the Stencil docs: https://stenciljs.com/docs/output-targets#primary-package-output-target-validation`);
                }
                ...
                logValidationWarning(buildCtx, `Your Stencil project has not assigned a primary package output target. Stencil recommends that you assign a primary output target so it can validate values for fields in your project's 'package.json'. You can read more about primary package output targets in the Stencil docs: https://stenciljs.com/docs/output-targets#primary-package-output-target-validation`);
            }
        }
        // Log a warning if any targets that cannot be validated were marked as "primary"
        if (nonPrimaryTargets.length && nonPrimaryTargets.some((ref) => ref.isPrimaryPackageOutputTarget)) {
            logValidationWarning(buildCtx, `Your Stencil project has assigned one or more ineligible output targets as the primary package output target. No validation will take place. Please remove the 'isPrimaryPackageOutputTarget' flag from the following output targets in your Stencil config: ${nonPrimaryTargets
                .filter((ref) => ref.isPrimaryPackageOutputTarget === true)
                .map((ref) => ref.type)
                .join(', ')}. You can read more about primary package output targets in the Stencil docs: https://stenciljs.com/docs/output-targets#primary-package-output-target-validation`);
        }
    }
    else {
        if (config.outputTargets.some((ref) => ref.isPrimaryPackageOutputTarget)) {
            logValidationWarning(buildCtx, 'Your Stencil project has designated a primary package output target without enabling primary package validation for your project. Either set `validatePrimaryPackageOutputTarget: true` in your Stencil config or remove `isPrimaryPackageOutputTarget: true` from all output targets. You can read more about primary package output targets in the Stencil docs: https://stenciljs.com/docs/output-targets#primary-package-output-target-validation');
        }
    }
};
/**
 ...
 */
export const validateModulePath = (config, compilerCtx, buildCtx, recommendedOutputTargetConfig, targetToValidate) => {
    ...
    if (warningMessage === null || warningMessage === void 0 ? void 0 : warningMessage.length) {
 ...
 * @param targetToValidate The output target to validate against.
 */
export const validateTypesPath = (config, compilerCtx, buildCtx, recommendedOutputTargetConfig, targetToValidate) => {
    ...
    else if (warningMessage === null || warningMessage === void 0 ? void 0 : warningMessage.length) {
        packageJsonWarn(config, compilerCtx, buildCtx, warningMessage, `"types"`);
    }
};
const logValidationWarning = (buildCtx, message) => {
    const warning = buildWarn(buildCtx.diagnostics);
    warning.header = 'Stencil Config';
    warning.messageText = message;
};
//# sourceMappingURL=validate-primary-package-output-target.js.map