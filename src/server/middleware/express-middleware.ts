...
export function ssrMiddleware(middlewareConfig: d.MiddlewareConfig) {
  // load up the config
  middlewareConfig.config = compilerLoadConfig(middlewareConfig.config);
  ...
  try {
    ...
    logger.error(`ssrMiddleware, error loading srcIndexHtml`, e);
    process.exit(1);
  }

  // middleware fn
  return function(req: any, res: any) {
    logger.debug(`ssr request: ${req.url}`);
    ...
    renderer.hydrate({
      ...
      logger.printDiagnostics(results.diagnostics);
