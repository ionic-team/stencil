...
* [General](#general)
  ...
  * [`assetsDir` Removed from `@Component()` decorator](#assetsdir-removed-from-component-decorator)
...
Previously, sourcemaps had to be explicitly enabled by setting this flag to `true`.
...
#### `assetsDir` Removed from `@Component()` decorator
...
`customeElementsExportBehavior` is a new configuration option for the output target.
It allows users to configure the export behavior of the output target.
...
This behavior has been merged into the [`customElementsExportBehavior`](#add-customelementsexportbehavior-to-control-export-behavior) configuration field.
...
The `dist-custom-elements` differs from `dist-custom-elements-bundle`.
By default, it does not automatically define all a project's component's with the `CustomElementsRegistry`.
...
### Legacy Angular Output Target (`angular`)
...
### Stencil APIs
Stencil exposes a Node API for programmatically invoking the compiler.
