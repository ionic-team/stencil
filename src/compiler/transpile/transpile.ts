...
import { buildError, catchError, isSassFile, isStylusFile, normalizePath } from '../util';
...
function processIncludedStyles(config: BuildConfig, ctx: BuildContext, moduleFile: ModuleFile) {
  if (ctx.isChangeBuild && !ctx.changeHasSass && !ctx.changeHasStylus && !ctx.changeHasCss) {
  ...
  modeNames.forEach(modeName => {
    ...
    if (modeMeta.absolutePaths) {
      modeMeta.absolutePaths.forEach(absoluteStylePath => {
        ...
        } else if (isStylusFile(absoluteStylePath)) {
          // this componet mode has a sass file, let's see which
          // sass files are included in it
          promises.push(
            getIncludedStylusFiles(config, ctx.diagnostics, moduleFile, absoluteStylePath)
          );
        }
...
function getIncludedStylusFiles(config: BuildConfig, diagnostics: Diagnostic[], moduleFile: ModuleFile, stylusFilePath: string) {
  return new Promise(resolve => {
    stylusFilePath = normalizePath(stylusFilePath);
    ...
    moduleFile.includedStylusFiles = moduleFile.includedStylusFiles || [];

    if (moduleFile.includedStylusFiles.indexOf(stylusFilePath) === -1) {
      moduleFile.includedStylusFiles.push(stylusFilePath);
    }

    if (!config.sys.stylus) {
      throw 'getIncludedStylusFiles: stylus config object must be defined in config.sys'
    }

    const strToRender: string = config.sys.fs.readFileSync(stylusFilePath, 'utf-8');
    // (str, options, fn)
    ...
    config.sys.stylus.render(strToRender, stylusConfig, (err, result) => {
      if (err) {
        ...
        moduleFile.includedStylusFiles.push(stylusConfig.finename);
      }
