import { catchError, dashToPascalCase, generatePreamble, getSourceMappingUrlForEndOfFile, hasError, isOutputTargetDistCustomElements, isString, join, rollupToStencilSourceMap, } from '@utils';
 ...
 */
export const outputCustomElements = async (config, compilerCtx, buildCtx) => {
    if (!config.buildDist) {
        return;
    }
 ...
 */
export const getBundleOptions = (config, buildCtx, compilerCtx, outputTarget) => ({
    id: 'customElements',
    platform: 'client',
 ...
 */
export const bundleCustomElements = async (config, compilerCtx, buildCtx, outputTarget) => {
    try {
 ...
 */
export const addCustomElementInputs = (buildCtx, bundleOpts, outputTarget) => {
 ...
 */
export const generateEntryPoint = (outputTarget, cmpImports = [], cmpExports = [], cmpNames = []) => {
    const body = [];
    const imports = [];
    const exports = [];
    // Exports that are always present
    exports.push(`export { getAssetPath, setAssetPath, setNonce, setPlatformOptions } from '${STENCIL_INTERNAL_CLIENT_ID}';`, `export * from '${USER_INDEX_ENTRY_ID}';`);
 ...
 */
const getCustomBeforeTransformers = (config, compilerCtx, components, outputTarget) => {
    const transformOpts = {
    ...
    customBeforeTransformers.push(nativeComponentTransform(compilerCtx, transformOpts), proxyCustomElement(compilerCtx, transformOpts), removeCollectionImports(compilerCtx));
