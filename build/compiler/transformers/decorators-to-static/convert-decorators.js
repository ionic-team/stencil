...
import { retrieveTsDecorators, retrieveTsModifiers, tsPropDeclNameAsString, updateConstructor, } from '../transform-utils';
...
import { CLASS_DECORATORS_TO_REMOVE, CONSTRUCTOR_DEFINED_MEMBER_DECORATORS, MEMBER_DECORATORS_TO_REMOVE, } from './decorators-constants';
 ...
 */
export const convertDecoratorsToStatic = (config, diagnostics, typeChecker, program) => {
    return (transformCtx) => {
        const visit = (node) => {
 ...
 */
const visitClassDeclaration = (config, diagnostics, typeChecker, program, classNode) => {
    var _a, _b, _c;
    ...
    if (decoratedMembers.length > 0) {
        ...
        methodDecoratorsToStatic(config, diagnostics, classNode, decoratedMembers, typeChecker, program, filteredMethodsAndFields);
 ...
 */
export const filterDecorators = (decorators, excludeList) => {
    if (decorators) {
        const updatedDecoratorList = decorators.filter((dec) => {
            ...
            const decoratorName = ts.isCallExpression(dec.expression) &&
...
function handleClassFields(classNode, classMembers, typeChecker) {
    ...
    for (const member of classMembers) {
        if (shouldInitializeInConstructor(member) && ts.isPropertyDeclaration(member)) {
            ...
            statements.push(ts.factory.createExpressionStatement(ts.factory.createBinaryExpression(ts.factory.createPropertyAccessExpression(ts.factory.createThis(), ts.factory.createIdentifier(memberName)), ts.factory.createToken(ts.SyntaxKind.EqualsToken), 
...
//# sourceMappingURL=convert-decorators.js.map