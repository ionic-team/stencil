(window.Ionic = window.Ionic || {}).components = {
  "ion-badge": [
    {
      "ios": "0",
      "md": "1",
...
(function (window, document) {
    ...
    function DomController(window) {
        var readCBs = [];
        var writeCBs = [];
        var rafPending = void 0;
        function domRead(cb) {
            readCBs.push(cb);
            if (!rafPending) {
                rafQueue();
    ...
    function NextTickController(window) {
        /* Adopted from Vue.js, MIT, https://github.com/vuejs/vue */
        var callbacks = [];
        var pending = false;
        var timerFunc = void 0;
        var isIOS = /iphone|ipad|ipod|ios/.test(window.navigator.userAgent.toLowerCase());
    ...
    function PlatformClient(win, doc, ionic, staticDir, domCtrl, nextTickCtrl) {
        ...
        ionic.loadComponents = function loadComponents(bundleId, cmpModeData) {
    ...
    var nextTickCtrl = NextTickController(window);
    var plt = PlatformClient(window, document, ionic, ionic.staticDir, domCtrl, nextTickCtrl);
    var renderer = Renderer(plt);
    var configCtrl = ConfigController(ionic.config || {});
