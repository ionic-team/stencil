 ...
 */
export const proxyCustomElement = (compilerCtx, transformOpts) => {
    return () => {
        return (tsSourceFile) => {
            ...
            for (const [stmtIndex, stmt] of tsSourceFile.statements.entries()) {
                if (ts.isVariableStatement(stmt)) {
                    for (const [declarationIndex, declaration] of stmt.declarationList.declarations.entries()) {
                        ...
                        const updatedVariableStatement = ts.factory.updateVariableStatement(stmt, stmt.modifiers, updatedDeclarationList);
...
//# sourceMappingURL=proxy-custom-element-function.js.map