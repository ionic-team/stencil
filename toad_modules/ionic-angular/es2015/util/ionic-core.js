 ...
 * @return {?}
 */
export function setupCore(config, plt, domCtrl, zone) {
    return function () {
        const /** @type {?} */ win = plt.win();
        const /** @type {?} */ doc = plt.doc();
        const /** @type {?} */ ionic = win['Ionic'];
...
function getNextTickController(zone, ua) {
    ...
    const /** @type {?} */ isIOS = /iphone|ipad|ipod|ios/.test(ua);
    const /** @type {?} */ noop = () => { };
    const /** @type {?} */ promise = Promise.resolve();
    const /** @type {?} */ logError = (err) => { console.error(err); };
    let /** @type {?} */ pending = false;
    /**
     * @return {?}
     */
    function nextTickHandler() {
        pending = false;
        const /** @type {?} */ copies = callbacks.slice(0);
        callbacks.length = 0;
        for (let /** @type {?} */ i = 0; i < copies.length; i++) {
            copies[i]();
        }
    }
    /**
