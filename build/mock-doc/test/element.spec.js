...
describe('element', () => {
    ...
    `);
        const clonedWin = cloneWindow(win);
        const elm = clonedWin.document.getElementById('test');
        expect(elm.content).toBe('');
        expect(elm).toEqualHtml(`<meta id="test">`);
        elm.content = 'value';
        expect(elm.content).toBe('value');
    ...
    describe('append', () => {
        ...
        it('one text', () => {
            ...
            expect(elm.childNodes[0].data).toEqual('text');
        });
        it('multiple texts', () => {
            const elm = doc.createElement('div');
            ...
            expect(elm.childNodes[0].data).toEqual('text');
            expect(elm.childNodes[1].data).toEqual('some text');
        });
        it('mixed', () => {
            const elm = doc.createElement('div');
            const child = doc.createElement('div');
            elm.append('text', 12, child, null);
            expect(elm.childNodes.length).toEqual(4);
            expect(elm.childNodes[0].data).toEqual('text');
            expect(elm.childNodes[1].data).toEqual('12');
            expect(elm.childNodes[2]).toEqual(child);
            expect(elm.childNodes[3].data).toEqual('null');
        });
    });
    describe('prepend', () => {
        it('before node', () => {
        ...
        it('before text', () => {
            ...
            expect(elm.childNodes[1].data).toEqual('12');
            expect(elm.childNodes[2]).toEqual(child);
            expect(elm.childNodes[3].data).toEqual('original');
        });
    });
