...
describe('validate-package-json', () => {
    ...
    describe('collection', () => {
        ...
        it('should produce a warning if the supplied path does not match the recommended path', () => {
            ...
            v.validateCollection(config, compilerCtx, buildCtx, collectionOutputTarget);
            expect(buildCtx.diagnostics[0].messageText).toBe(`package.json "collection" property is required when generating a distribution and must be set to: ${normalizePath('dist/collection/collection-manifest.json', false)}`);
            expect(buildCtx.diagnostics[0].level).toBe('warn');
        });
...
//# sourceMappingURL=validate-package-json.spec.js.map