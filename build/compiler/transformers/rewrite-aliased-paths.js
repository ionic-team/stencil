 ...
 */
export function rewriteAliasedDTSImportPaths(transformCtx) {
 ...
 * @returns a TypeScript transformer
 */
export function rewriteAliasedSourceFileImportPaths(transformCtx) {
    const compilerHost = ts.createCompilerHost(transformCtx.getCompilerOptions());
    return (tsSourceFile) => {
...
function rewriteAliasedImport(compilerHost, transformCtx, sourceFilePath, node) {
    ...
    const extensionRegex = new RegExp(Object.values(ts.Extension)
    ...
    return transformCtx.factory.updateImportDeclaration(node, retrieveTsModifiers(node), node.importClause, transformCtx.factory.createStringLiteral(
    ...
    // above.
    importPath.startsWith('.') ? importPath : './' + importPath), node.assertClause);
}
//# sourceMappingURL=rewrite-aliased-paths.js.map