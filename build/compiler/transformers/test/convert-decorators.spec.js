...
describe('convert-decorators', () => {
    ...
    `);
        ...
        // in the constructor
        expect(await formatCode(t.outputText)).toBe(await c `export class CmpA {
        constructor() {
    ...
    `);
        ...
        // removed from the class body correctly and replaced with an initializer
        // in the constructor
        expect(await formatCode(t.outputText)).toContain(`  constructor() {
    this.val = undefined;
  }`);
    });
    ...
    `);
        ...
        // removed from the class body correctly and replaced with an initializer
        // in the constructor
        expect(await formatCode(t.outputText)).toBe(await c `export class CmpB {
        constructor() {
          this.count = 0;
        }
        static get is() {
    ...
    it('should add a super call to the constructor if necessary', async () => {
        const t = transpileModule(`@Component({tag: 'cmp-a'})
      export class CmpA extends Foobar {
        @State() count: number = 0;
      }
    `);
        expect(await formatCode(t.outputText)).toBe(await c `export class CmpA extends Foobar {
        constructor() {
          super();
          this.count = 0;
        }
    ...
    export class MyComponent {
      constructor() {
        ...
        expect(await formatCode(t.outputText)).toBe(await c `export class MyComponent {
        constructor() {
          console.log('boop');
        }
        static get is() {
    ...
    it('should allow user to initialize field in an existing constructor w/ @Prop', async () => {
        const t = transpileModule(`@Component({
      tag: 'my-component',
    })
    export class MyComponent {
      @Prop() count: number;
    ...
    export class Example implements FooBar {
      ...
      constructor() {
        ...
        expect(await formatCode(t.outputText)).toBe(await c `export class Example {
        constructor() {
          this.classProps = ["variant", "theme"];
        }}`);
    });
    ...
    `);
        expect(await formatCode(await formatCode(t.outputText))).toBe(await c `export class CmpA {
        constructor() {
          this.count = 0;
        }
        static get is() {
    ...
    describe('filterDecorators', () => {
        it.each([[[]], [['ExcludedDecorator']]])('returns undefined when no decorators are provided', (excludeList) => {
            const filteredDecorators = filterDecorators(undefined, excludeList);
            expect(filteredDecorators).toBeUndefined();
        });
        it.each([[[]], [['ExcludedDecorator']]])('returns undefined for an empty list of decorators', (excludeList) => {
        ...
        it("doesn't return any decorators when all decorators in the exclude list", () => {
            ...
            const customDecorator = ts.factory.createDecorator(ts.factory.createCallExpression(ts.factory.createIdentifier('CustomProp'), undefined, []));
            // create '@Prop()' decorator
            const decorator = ts.factory.createDecorator(ts.factory.createCallExpression(ts.factory.createIdentifier('Prop'), undefined, []));
...
//# sourceMappingURL=convert-decorators.spec.js.map