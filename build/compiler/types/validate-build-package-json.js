import { COLLECTION_MANIFEST_FILE_NAME, isGlob, isOutputTargetDistCollection, isString, join, normalizePath, relative, } from '@utils';
...
export const validateBuildPackageJson = async (config, compilerCtx, buildCtx) => {
    ...
    await Promise.all(distCollectionOutputTargets.map((distCollectionOT) => validateDistCollectionPkgJson(config, compilerCtx, buildCtx, distCollectionOT)));
};
/**
...
export const validateCollection = (config, compilerCtx, buildCtx, outputTarget) => {
    if (outputTarget.collectionDir) {
        const collectionRel = normalizePath(join(relative(config.rootDir, outputTarget.collectionDir), COLLECTION_MANIFEST_FILE_NAME), false);
...
//# sourceMappingURL=validate-build-package-json.js.map