...
import { PRIMARY_PACKAGE_TARGET_CONFIGS, validateModulePath, validatePrimaryPackageOutputTarget, validateTypesPath, } from '../validate-primary-package-output-target';
describe('validatePrimaryPackageOutputTarget', () => {
    ...
    describe('check basic Stencil config scenarios', () => {
        ...
        it('should log a warning if no eligible targets were marked as `isPrimaryPackageOutputTarget`', () => {
            ...
            expect(buildCtx.diagnostics.length).toBe(1);
            expect(buildCtx.diagnostics[0].level).toEqual('warn');
            expect(buildCtx.diagnostics[0].messageText).toEqual(`Your Stencil project has not assigned a primary package output target. Stencil recommends that you assign a primary output target so it can validate values for fields in your project's 'package.json'. You can read more about primary package output targets in the Stencil docs: https://stenciljs.com/docs/output-targets#primary-package-output-target-validation`);
        });
        it('should log a warning if multiple targets were marked as `isPrimaryPackageOutputTarget`', () => {
            ...
            expect(buildCtx.diagnostics.length).toBe(1);
            expect(buildCtx.diagnostics[0].level).toEqual('warn');
            expect(buildCtx.diagnostics[0].messageText).toEqual(`Your Stencil config has multiple output targets with 'isPrimaryPackageOutputTarget: true'. Stencil does not support validating 'package.json' fields for multiple output targets. Please remove the 'isPrimaryPackageOutputTarget' flag from all but one of the following output targets: dist, dist-custom-elements. For now, Stencil will use the first primary target it finds. You can read more about primary package output targets in the Stencil docs: https://stenciljs.com/docs/output-targets#primary-package-output-target-validation`);
        });
    });
    describe('validateModulePath', () => {
        it('should log a warning if no module path is provided', () => {
            delete buildCtx.packageJson.module;
            const targetToValidate = {
                type: 'dist',
                dir: '/dist',
            };
            ...
            expect(buildCtx.diagnostics.length).toBe(1);
            expect(buildCtx.diagnostics[0].level).toEqual('warn');
            expect(buildCtx.diagnostics[0].messageText).toEqual(`package.json "module" property is required when generating a distribution. It's recommended to set the "module" property to: ./dist/index.js`);
        });
        describe.each([
            [
                {
            ...
            it('should log a warning if the set module path does not match the recommended path', () => {
                ...
                validateModulePath(config, compilerCtx, buildCtx, PRIMARY_PACKAGE_TARGET_CONFIGS[outputTarget.type], outputTarget);
                expect(buildCtx.diagnostics.length).toBe(1);
                expect(buildCtx.diagnostics[0].level).toEqual('warn');
                expect(buildCtx.diagnostics[0].messageText).toEqual(`package.json "module" property is set to "${buildCtx.packageJson.module}". It's recommended to set the "module" property to: ${recommendedPath}`);
            });
    ...
    describe('validateTypesPath', () => {
        ...
        let recommendedOutputTargetConfig;
        beforeEach(() => {
            targetToValidate = {
                type: 'dist-types',
                dir: '/dist/types',
        ...
        it('should log a warning if no types path is provided', () => {
            ...
            expect(buildCtx.diagnostics.length).toBe(1);
            expect(buildCtx.diagnostics[0].level).toEqual('warn');
            expect(buildCtx.diagnostics[0].messageText).toEqual(`package.json "types" property is required when generating a distribution. It's recommended to set the "types" property to: ./dist/types/index.d.ts`);
        });
        it('should log a warning if the types path does not have a ".d.ts" extension', () => {
            ...
            expect(buildCtx.diagnostics.length).toBe(1);
            expect(buildCtx.diagnostics[0].level).toEqual('warn');
            expect(buildCtx.diagnostics[0].messageText).toEqual(`package.json "types" file must have a ".d.ts" extension. The "types" property is currently set to: /dist/types/index.ts`);
        });
        it('should log a error if the types file cannot be accessed', () => {
            ...
            expect(buildCtx.diagnostics[0].messageText).toEqual(`package.json "types" property is set to "dist/types/index.d.ts" but cannot be found.`);
        });
        describe.each([
            [
                {
            ...
            it('should log a warning if the set types path does not match the recommended path', () => {
                ...
                validateTypesPath(config, compilerCtx, buildCtx, PRIMARY_PACKAGE_TARGET_CONFIGS[outputTarget.type], outputTarget);
                expect(buildCtx.diagnostics.length).toBe(1);
                expect(buildCtx.diagnostics[0].level).toEqual('warn');
                expect(buildCtx.diagnostics[0].messageText).toEqual(`package.json "types" property is set to "${buildCtx.packageJson.types}". It's recommended to set the "types" property to: ${recommendedPath}`);
            });
            it('should not log anything if the recommended path is used and accessible', () => {
                buildCtx.packageJson.types = recommendedPath;
                validateTypesPath(config, compilerCtx, buildCtx, PRIMARY_PACKAGE_TARGET_CONFIGS[outputTarget.type], targetToValidate);
                expect(buildCtx.diagnostics.length).toBe(0);
            });
        });
    });
});
//# sourceMappingURL=validate-primary-package-output-target.spec.js.map