 ...
 */
export function createLazyAttachInternalsBinding(cmp) {
    if (cmp.formAssociated && cmp.attachInternalsMemberName) {
        return [
            ...
            // ```
            hostRefElementInternalsPropAccess(), ts.factory.createBlock([
                ...
                ts.factory.createIdentifier(cmp.attachInternalsMemberName)), ts.factory.createToken(ts.SyntaxKind.EqualsToken), hostRefElementInternalsPropAccess())),
                ...
                ts.factory.createExpressionStatement(ts.factory.createBinaryExpression(ts.factory.createPropertyAccessExpression(ts.factory.createThis(), 
                // use the name set on the {@link d.ComponentCompilerMeta}
                ts.factory.createIdentifier(cmp.attachInternalsMemberName)), ts.factory.createToken(ts.SyntaxKind.EqualsToken), ts.factory.createCallExpression(ts.factory.createPropertyAccessExpression(ts.factory.createPropertyAccessExpression(ts.factory.createIdentifier(HOST_REF_ARG), ts.factory.createIdentifier('$hostElement$')), ts.factory.createIdentifier('attachInternals')), undefined, []))),
                // this `ts.factory` call produces the following:
                //
                // ```ts
                // hostRef.$hostElement$['s-ei'] = this.${ cmp.attachInternalsMemberName };
                // ```
                ts.factory.createExpressionStatement(ts.factory.createBinaryExpression(hostRefElementInternalsPropAccess(), ts.factory.createToken(ts.SyntaxKind.EqualsToken), ts.factory.createPropertyAccessExpression(ts.factory.createThis(), 
                // use the name set on the {@link d.ComponentCompilerMeta}
...
function hostRefElementInternalsPropAccess() {
    return ts.factory.createElementAccessExpression(ts.factory.createPropertyAccessExpression(ts.factory.createIdentifier(HOST_REF_ARG), ts.factory.createIdentifier('$hostElement$')), ts.factory.createStringLiteral('s-ei'));
}
