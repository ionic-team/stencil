...
import { COLLECTION_MANIFEST_FILE_NAME, flatOne, getStencilCompilerContext, normalizePath, sortBy } from '@utils';
import { isOutputTargetDistCollection } from './output-utils';
import { join, relative } from 'path';
import { typescriptVersion, version } from '../../version';

export const outputCollections = async (config: d.Config, buildCtx: d.BuildCtx) => {
  ...
  await Promise.all([
    writeJsFiles(config, moduleFiles, outputTargets),
    writeCollectionManifests(config, buildCtx, outputTargets),
  ]);

  timespan.finish(`generate collections finished`);
};

const writeJsFiles = (config: d.Config, moduleFiles: d.Module[], outputTargets: d.OutputTargetDistCollection[]) => {
  return Promise.all(moduleFiles.map((moduleFile) => writeModuleFile(config, moduleFile, outputTargets)));
};
...
const writeModuleFile = async (
  ...
  const jsContent = await getStencilCompilerContext().fs.readFile(moduleFile.jsFilePath);
  await Promise.all(
    outputTargets.map((o) => {
      const outputFilePath = join(o.collectionDir, relPath);
      return getStencilCompilerContext().fs.writeFile(outputFilePath, jsContent);
    })
  );
};
...
export const writeCollectionManifests = async (
  ...
  const collectionData = JSON.stringify(serializeCollectionManifest(config, buildCtx), null, 2);
  return Promise.all(outputTargets.map((o) => writeCollectionManifest(collectionData, o)));
};
...
const writeCollectionManifest = async (collectionData: string, outputTarget: d.OutputTargetDistCollection) => {
  ...
  await getStencilCompilerContext().fs.writeFile(collectionFilePath, collectionData);
};

export const serializeCollectionManifest = (config: d.Config, buildCtx: d.BuildCtx) => {
  ...
  const collectionManifest: d.CollectionManifest = {
    ...
    collections: serializeCollectionDependencies(),
    bundles: config.bundles.map((b) => ({
      components: b.components.slice().sort(),
    })),
  };
  if (config.globalScript) {
    const mod = getStencilCompilerContext().moduleMap.get(normalizePath(config.globalScript));
    if (mod) {
...
const serializeCollectionDependencies = (): d.CollectionDependencyData[] => {
  const collectionDeps = getStencilCompilerContext().collections.map((c) => ({
