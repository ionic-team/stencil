...
describe('ShadowCss', function () {
    function s(cssText, scopeId, commentOriginalSelector = false) {
    ...
    describe(':host', () => {
        ...
        it('should not replace the selector in a `@supports` rule', () => {
            expect(s('@supports selector(:host()) {:host {color: red; }}', 'a')).toEqual('@supports selector(:host()) {.a-h {color:red;}}');
        });
    });
    describe(':host-context', () => {
        ...
        it('should not replace the selector in a `@supports` rule', () => {
            expect(s('@supports selector(:host-context(.class1)) {:host-context(.class1) {color: red; }}', 'a')).toEqual('@supports selector(:host-context(.class1)) {.class1.a-h, .class1 .a-h {color:red;}}');
        });
        ``;
    });
    describe('::slotted', () => {
        ...
        it('should handle :host complex selector', () => {
            ...
            expect(r).toEqual('.sc-ion-tag-h >.sc-ion-tag-s > *:nth-of-type(2n - 1), .sc-ion-tag-h > .sc-ion-tag-s > *:nth-of-type(2n - 1) {}');
        });
        it('should handle host-context complex selector', () => {
            const r = s(':host-context(.red) > ::slotted(*:nth-of-type(2n - 1)) {}', 'sc-ion-tag');
            expect(r).toEqual('.sc-ion-tag-h.red >.sc-ion-tag-s > *:nth-of-type(2n - 1), .sc-ion-tag-h.red > .sc-ion-tag-s > *:nth-of-type(2n - 1), .red .sc-ion-tag-h >.sc-ion-tag-s > *:nth-of-type(2n - 1), .red .sc-ion-tag-h > .sc-ion-tag-s > *:nth-of-type(2n - 1) {}');
        });
        ...
        it('should not replace the selector in a `@supports` rule', () => {
            expect(s('@supports selector(::slotted(*)) {::slotted(*) {color: red; }}', 'sc-cmp')).toEqual('@supports selector(::slotted(*)) {.sc-cmp-s > * {color:red;}}');
        });
    });
...
//# sourceMappingURL=scope-css.spec.js.map