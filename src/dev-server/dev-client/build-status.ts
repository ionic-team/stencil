...
import { onBuildStatus } from './build-events';


export function initBuildStatus(win: d.DevClientWindow, doc: Document) {
...
function updateBuildStatus(doc: Document, status: d.BuildStatus) {
  const iconElms = getFavIcons(doc);
...
export function updateFavIcon(linkElm: HTMLLinkElement, status: d.BuildStatus) {
  if (status === 'pending') {
...
function getFavIcons(doc: Document) {
