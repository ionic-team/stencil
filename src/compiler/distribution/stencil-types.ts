...
export async function copyStencilCoreDts(config: d.Config, compilerCtx: d.CompilerCtx) {
  ...
  await Promise.all(typesOutputTargets.map(async typesOutputTarget => {
    await copyStencilCoreDtsOutput(compilerCtx, typesOutputTarget);
  }));
}


async function copyStencilCoreDtsOutput(compilerCtx: d.CompilerCtx, outputTarget: d.OutputTargetDist) {
