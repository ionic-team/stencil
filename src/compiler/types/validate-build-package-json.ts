...
export const validateBuildPackageJson = async (config: d.Config, buildCtx: d.BuildCtx) => {
  if (config.watch) {
    return;
  }
  ...
  await Promise.all([
    ...outputTargets.map((outputsTarget) => {
      return validatePackageJsonOutput(config, buildCtx, outputsTarget);
    }),
    ...typesOutputTargets.map((outputTarget) => {
      return validateTypes(config, buildCtx, outputTarget);
    }),
  ]);
};
...
const validatePackageJsonOutput = async (
  ...
  await Promise.all([
    validatePackageFiles(config, buildCtx, outputTarget),
    validateMain(config, buildCtx, outputTarget),
    validateModule(config, buildCtx, outputTarget),
    validateCollection(config, buildCtx, outputTarget),
    validateBrowser(config, buildCtx),
  ]);
};
...
export const validatePackageFiles = async (
  ...
  if (!config.devMode && Array.isArray(buildCtx.packageJson.files)) {
    ...
    if (!containsDistDir) {
      ...
      packageJsonWarn(config, buildCtx, msg, `"files"`);
      return;
    }
    ...
    await Promise.all(
      buildCtx.packageJson.files.map(async (pkgFile) => {
        if (!isGlob(pkgFile)) {
          ...
          const hasAccess = await getStencilCompilerContext().fs.access(absPath);
          if (!hasAccess) {
            const msg = `Unable to find "${pkgFile}" within the package.json "files" array.`;
            packageJsonError(config, buildCtx, msg, `"${pkgFile}"`);
          }
        }
      })
    );
...
export const validateMain = (config: d.Config, buildCtx: d.BuildCtx, outputTarget: d.OutputTargetDistCollection) => {
  ...
  if (!isString(buildCtx.packageJson.main) || buildCtx.packageJson.main === '') {
    ...
    packageJsonWarn(config, buildCtx, msg, `"main"`);
...
export const validateModule = (config: d.Config, buildCtx: d.BuildCtx, outputTarget: d.OutputTargetDistCollection) => {
  ...
  if (!isString(currentModule)) {
    ...
    packageJsonWarn(config, buildCtx, msg, `"module"`);
  } else if (
...
export const validateTypes = async (config: d.Config, buildCtx: d.BuildCtx, outputTarget: d.OutputTargetDistTypes) => {
  ...
  if (!isString(buildCtx.packageJson.types) || buildCtx.packageJson.types === '') {
    ...
    if (!typesFileExists) {
      ...
      packageJsonError(config, buildCtx, msg, `"types"`);
    }
  }
};
...
export const validateCollection = (
  ...
  if (outputTarget.collectionDir) {
    ...
    if (!buildCtx.packageJson.collection || normalizePath(buildCtx.packageJson.collection) !== collectionRel) {
      ...
      packageJsonWarn(config, buildCtx, msg, `"collection"`);
    }
  }
};

export const validateBrowser = (config: d.Config, buildCtx: d.BuildCtx) => {
  if (isString(buildCtx.packageJson.browser)) {
    ...
    packageJsonWarn(config, buildCtx, msg, `"browser"`);
  }
};

const packageJsonError = (config: d.Config, buildCtx: d.BuildCtx, msg: string, warnKey: string) => {
  const err = buildJsonFileError(buildCtx.diagnostics, config.packageJsonFilePath, msg, warnKey);
  err.header = `Package Json`;
  return err;
};

const packageJsonWarn = (config: d.Config, buildCtx: d.BuildCtx, msg: string, warnKey: string) => {
  const warn = buildJsonFileError(buildCtx.diagnostics, config.packageJsonFilePath, msg, warnKey);
