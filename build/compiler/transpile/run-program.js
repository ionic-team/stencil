import { getComponentsFromModules, isOutputTargetDistTypes, join, loadTypeScriptDiagnostics, normalizePath, relative, } from '@utils';
...
export const runTsProgram = async (config, compilerCtx, buildCtx, tsBuilder) => {
    ...
    const emitCallback = (emitFilePath, data, _w, _e, tsSourceFiles) => {
        ...
        else if (emitFilePath.endsWith('.e2e.d.ts') ||
 ...
 * written
 */
export const getRelativeDts = (config, srcPath, emitDtsPath) => {
    const parts = [];
    for (let i = 0; i < 30; i++) {
