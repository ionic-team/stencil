 ...
 */
export function createNativeAttachInternalsBinding(cmp) {
    if (cmp.formAssociated && cmp.attachInternalsMemberName) {
        return [
            ...
            ts.factory.createIdentifier(cmp.attachInternalsMemberName)), ts.factory.createToken(ts.SyntaxKind.EqualsToken), ts.factory.createCallExpression(ts.factory.createPropertyAccessExpression(ts.factory.createThis(), ts.factory.createIdentifier('attachInternals')), undefined, []))),
        ];
    }
    else {
        return [];
