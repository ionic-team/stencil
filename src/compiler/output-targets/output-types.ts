...
export const outputTypes = async (config: d.Config, buildCtx: d.BuildCtx) => {
  ...
  await Promise.all(outputTargets.map((outputsTarget) => generateTypes(config, buildCtx, outputsTarget)));
