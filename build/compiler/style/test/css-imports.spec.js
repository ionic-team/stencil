...
import { getCssImports, isCssNodeModule, isLocalCssImport, parseCssImports, replaceImportDeclarations, } from '../css-imports';
describe('css-imports', () => {
    ...
    describe('parseCssImports', () => {
        ...
        it('should merge in imported files w/ child imports', async () => {
            ...
            const result = await parseCssImports(config, compilerCtx, buildCtx, mainFilePath, mainFilePath, files[mainFilePath], []);
            // CSS from child and grandchild are merged in
            expect(result.styleText).toBe('div { display: flex } :host { color: red; }');
        });
    });
});
//# sourceMappingURL=css-imports.spec.js.map