...
const createElm = (oldParentVNode: d.VNode, newParentVNode: d.VNode, childIndex: number, parentElm: d.RenderNode, i?: number, elm?: d.RenderNode, childNode?: d.RenderNode, newVNode?: d.VNode, oldVNode?: d.VNode) => {
  ...
  if (isDef(newVNode.vtext)) {
    ...
    elm = newVNode.elm = ((BUILD.svg && (isSvgMode || newVNode.vtag === 'svg')) ?
                      document.createElementNS('http://www.w3.org/2000/svg', newVNode.vtag as string) :
    ...
    if (newVNode.vchildren) {
      for (i = 0; i < newVNode.vchildren.length; ++i) {
        ...
        if (childNode) {
          ...
          //   elm.appendChild(document.createComment('s.' + ssrId + '.' + i));
          // }
...
export const vdomRender = (hostElement: d.HostElement, cmpMeta: d.ComponentRuntimeMeta, oldVNode: d.VNode, renderFnResults: any, hostDataFnResults?: any) => {
  ...
  if (BUILD.slotPolyfill) {
    useNativeShadowDom = supportsShadowDom && cmpMeta.shadowDomEncapsulation;
  }
    ...
    if (checkSlotRelocate) {
      ...
      for (let i = 0; i < relocateNodes.length; i++) {
        ...
        if (!relocateNode.nodeToRelocate['s-ol']) {
          ...
          const orgLocationNode = document.createTextNode('') as any;
